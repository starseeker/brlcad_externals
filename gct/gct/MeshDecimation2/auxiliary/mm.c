...
static int mmNumaInit()
{
 ...
 #elif MM_WINDOWS
  int nodecount, nodeindex, cpuindex;
  unsigned long long nodememory;
  unsigned short cpunodeindex;
  SYSTEM_INFO sysinfo;
  nodecount = GetNumaHighestNodeNumber();
  if( nodecount == 0 )
    return 0;
  mmcontext.numaflag = 1;
  GetSystemInfo( &sysinfo );
  mmcontext.cpucount = sysinfo.dwNumberOfProcessors;
  if( mmcontext.cpucount >= MM_CPU_COUNT_MAXIMUM )
  {
    fprintf( stderr, "CPU count %d exceeds %d, increase MM_CPU_COUNT_MAXIMUM in mm.h and try again.\n", mmcontext.cpucount, MM_CPU_COUNT_MAXIMUM );
    exit( 1 );
  }
  mmcontext.nodecount = nodecount;
  if( mmcontext.nodecount >= MM_NODE_COUNT_MAXIMUM )
  {
    fprintf( stderr, "Node count %d exceeds %d, increase MM_NODE_COUNT_MAXIMUM in mm.h and try again.\n", mmcontext.nodecount, MM_NODE_COUNT_MAXIMUM );
    exit( 1 );
  ...
  {
    nodememory = 0;
    GetNumaAvailableMemoryNodeEx( nodeindex, &nodememory );
    mmcontext.nodesize[nodeindex] = nodememory;
    mmcontext.sysmemory += mmcontext.nodesize[nodeindex];
    mmcontext.nodecpucount[nodeindex] = 0;
  }
  for( cpuindex = 0 ; cpuindex < mmcontext.cpucount ; cpuindex++ )
  {
    GetNumaProcessorNode( cpuindex, &cpunodeindex );
    mmcontext.cpunode[ cpuindex ] = cpunodeindex;
    mmcontext.nodecpucount[cpunodeindex]++;
  }
 #endif
  return 1;
}
...
#endif
#if MM_UNIX && defined(_SC_PHYS_PAGES)
...
void mmPrintNumaTopology()
{
  int nodeindex, cpuindex;
  printf( "NUMA nodes : %d\n", mmcontext.nodecount );
  for( nodeindex = 0 ; nodeindex < mmcontext.nodecount ; nodeindex++ )
    printf( "  NUMA node %d, size %ld, CPU count %d\n", nodeindex, (long)mmcontext.nodesize[nodeindex], mmcontext.nodecpucount[nodeindex] );
  printf( "CPUs : %d\n", mmcontext.cpucount );
  for( cpuindex = 0 ; cpuindex < mmcontext.cpucount ; cpuindex++ )
...
void mmThreadBindToNode( int nodeindex )
{
  ...
  if( mmcontext.numaflag )
  {
    ...
    for( cpuindex = 0 ; cpuindex < mmcontext.cpucount ; cpuindex++ )
    {
      if( mmcontext.cpunode[ cpuindex ] == nodeindex )
        cpumask |= (unsigned long long)1 << cpuindex;
    }
  }
 #endif
  SetThreadAffinityMask( handle, cpumask );
#endif
  return;
}
...
void mmThreadBindToCpu( int cpuindex )
{
  ...
  HANDLE *handle;
  handle = GetCurrentThread();
  SetThreadAffinityMask( handle, (unsigned long long)1 << cpuindex );
#endif
  return;
}
...
void *mmNodeAlloc( int nodeindex, size_t size )
{
 ...
 #elif MM_WIN32
  if( mmcontext.numaflag )
    return VirtualAllocExNuma( GetCurrentProcess(), NULL, size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE, nodeindex );
 #endif
#endif
  return malloc( size );
}
...
void mmNodeMap( int nodeindex, void *start, size_t bytes )
{
 ...
 #elif MM_WIN32
  /* How the hell do you do that on Windows?! */
 #endif
#endif
  return;
}

static void *mmNodeRelayAlloc( void *relayvalue, size_t bytes MM_PARAMS )
{
#ifdef MM_NUMA
 #if MM_LINUX
  if( mmcontext.numaflag )
    return numa_alloc_onnode( bytes, (int)((intptr_t)relayvalue) );
 #elif MM_WIN32
  if( mmcontext.numaflag )
    return VirtualAllocExNuma( GetCurrentProcess(), NULL, size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE, (int)((intptr_t)relayvalue) );
 #endif
#endif
  return malloc( bytes );
}

static void mmNodeRelayFree( void *relayvalue, void *v, size_t bytes MM_PARAMS )
{
#ifdef MM_NUMA
 #if MM_LINUX
  if( mmcontext.numaflag )
