...
static inline CC_ALWAYSINLINE int fmGlyphHashEntryValid( void *entry )
{
  return ( ((fmGlyph *)entry)->glyphkey != 0 );
}

static inline CC_ALWAYSINLINE uint32_t fmGlyphHashEntryKey( void *entry )
{
  return ccHash32Int64Inline( ((fmGlyph *)entry)->glyphkey );
}

static inline CC_ALWAYSINLINE int fmGlyphHashEntryCmp( void *entry, void *entryref )
{
  uint64_t glyphkey;
...
static inline CC_ALWAYSINLINE int fmKernCacheEntryValid( void *entry )
{
  return ( ((fmKernCacheEntry *)entry)->value != 0 );
}

static inline CC_ALWAYSINLINE uint32_t fmKernCacheEntryKey( void *entry )
{
  int64_t value;
...
static inline CC_ALWAYSINLINE int fmKernCacheEntryCmp( void *entry, void *entryref )
{
  int64_t value;
  value = ((fmKernCacheEntry *)entry)->value & FM_KERNCACHE_CODE_MASK;
  if( value == 0 )
...
static fmGlyph *fmGetGlyph( fmManager *fm, fmFont *font, uint32_t codepoint, int size, int subpixel, int processindex, int extrapadding )
{
  ...
  else
  {
    ...
    if( !( fmtt_buildGlyphBitmap( font, glyphindex, size, subpixel, &advance, &x0, &y0, &x1, &y1 ) ) )
    {
      if( codepoint != FM_GLYPH_CODEPOINT_REPLACEMENT )
        return fmGetGlyph( fm, font, FM_GLYPH_CODEPOINT_REPLACEMENT, size, subpixel, processindex, extrapadding );
      return 0;
    }
  }
  glyphwidth = x1 - x0;
...
static inline void fmAddGlyphAdvance( int *x, int *subpixel, fmGlyph *glyph )
{
...
int fmDrawTextWithCursor( fmManager *fm, int x, int y, const char *string, int stringlength, int cursoroffset )
{
  ...
  for( index = 0 ; ; index++ )
  {
    ...
    {
      glyph = fmGetGlyph( fm, font, FM_GLYPH_CODEPOINT_CURSOR, state->size, subpixel, processindex, extrapadding );
      if( glyph )
...
int fmGetTextWidth( fmManager *fm, const char *string, int stringlength )
{
  ...
  for( index = 0 ; index < stringlength ; index++ )
  {
    ...
    if( glyph )
    {
      ...
      fmAddGlyphAdvance( &advance, &subpixel, glyph );
    }
    prevcodepoint = codepoint;
  }
...
float fmPathDrawCharacter( fmManager *fm, fmPathDraw *pathdraw, float x, float y, float vectorx, float vectory, int unicode )
{
  ...
  glyph = fmGetGlyph( fm, font, unicode, state->size, 0, processindex, extrapadding );
  if( glyph )
  {
    subpixel = 0;
